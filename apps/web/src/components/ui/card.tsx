...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-lg border border-border/70 bg-card/80 text-card-foreground shadow-[0_18px_65px_-48px_rgba(0,0,0,0.95)] backdrop-blur-xl',
        className,
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn('text-lg font-semibold leading-snug sm:text-xl', className)} {...props} />
  ),
);
